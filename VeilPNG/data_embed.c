...
int embed_data_in_png(const TCHAR* png_path, const TCHAR* data_path, const TCHAR* output_path, const TCHAR* password) {
    ...
    memcpy(ptr, &file_name_length, sizeof(unsigned int));  // Copy file_name_length
    ptr += sizeof(unsigned int);
    memcpy(ptr, file_name_utf8, file_name_length);  // Copy file_name_utf8
    ptr += file_name_length;
    memcpy(ptr, data, data_size);  // Copy data
    ...
    // Append HMAC to compressed data
    ...
    // Step 9: Encrypt the data with HMAC
    ...
    // Step 10: Insert encrypted data into PNG
    ...
    // Step 11: Write new PNG data to file
...
int extract_data_from_png(const TCHAR* png_path, const TCHAR* output_folder, const TCHAR* password, TCHAR* extracted_file_name) {
    ...
    size_t file_data_size = decompressed_size - file_name_length - sizeof(unsigned int);
