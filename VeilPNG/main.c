...
#include <stdlib.h>
#include <string.h>
#include <sal.h>       // Include this header for annotations
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    int buttonWidth = 130;      // Uniform button width
    int buttonHeight = 30;

    // Positions for the three buttons
    ...
    // Extract Veil Button (now in the middle)
    ...
    // Instructions Button (now on the right)
    hInstructionsButton = CreateWindow(_T("BUTTON"), _T("Instructions"), WS_VISIBLE | WS_CHILD,
        startX + 2 * (buttonWidth + buttonSpacing), buttonYPosition, buttonWidth, buttonHeight, hwnd, (HMENU)IDC_INSTRUCTIONS_BUTTON, hInst, NULL);
    ...
    hProgressBar = CreateWindowEx(0, PROGRESS_CLASS, NULL, WS_CHILD,
        50, 210, 400, 20, hwnd, NULL, hInst, NULL);
    ...
    hStatusText = CreateWindow(_T("STATIC"), _T(""), WS_VISIBLE | WS_CHILD | SS_LEFT | SS_EDITCONTROL,
        10, 240, 480, 40, hwnd, NULL, hInst, NULL);
    ...
    hByAmadonText = CreateWindow(_T("STATIC"), _T("by Amadon"), WS_VISIBLE | WS_CHILD | SS_CENTER,
        0, 290, 500, 20, hwnd, NULL, hInst, NULL);
...
void ShowInstructions(HWND hwndParent) {
    ...
    if (!isRegistered) {
        ...
        if (!RegisterClass(&wc)) {
            // MessageBox(hwndParent, _T("Failed to register instructions window class."), _T("Error"), MB_OK | MB_ICONERROR);
            return;
        }
        isRegistered = TRUE;
    }
    ...
    if (!hwndInstructions) {
        // MessageBox(hwndParent, _T("Failed to create instructions window."), _T("Error"), MB_OK | MB_ICONERROR);
        return;
    }
...
DWORD WINAPI ExtractVeilThread(LPVOID param) {
    ...
    // Update progress bar
    SendMessage(hProgressBar, PBM_SETPOS, 50, 0);
    PostMessage(hwnd, WM_UPDATE_STATUS_TEXT, 0, (LPARAM)_tcsdup(_T("Decrypting data...")));
